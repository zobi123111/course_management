                                    ...
                                    <div id="lesson-{{ $eventLesson->id }}" class="accordion-collapse collapse show" data-bs-parent="#faq-group-2">
                                        <div class="accordion-body">
                                                        ...
                                                        <div class="grade_here_cont">
                                                            ...
                                                            <table>
                                                                <tbody>
                                                                    ...
                                                                    <tr>
                                                                        <td class="{{ $disableGrading ? 'disabled-td' : '' }}">
                                            ...
                                            <div class="collapse mt-2" id="comment-box-{{ $sublesson->id }}">
                                                <textarea name="task_comments[{{ $lesson->id ?? '' }}][{{ $sublesson->id ?? '' }}]" rows="3" class="form-control" placeholder="Add your remarks or feedback here..." @if($isDeferred) readonly title="Deferred: You cannot edit this comment." @endif {{ $disableGrading ? 'disabled' : '' }}>{{ old("task_comments.$lesson->id.$sublesson->id.$student->id", $selectedComment) }}</textarea>
                                            </div>
                                            @endforeach
                                            ...
                                            <div class="accordion-card">
                                                ...
                                                <div id="comptency-{{ $eventLesson->id }}" class="accordion-collapse collapse">
                                                    ...
                                                    <div class="accordion-body">
                                                        ...
                                                        // Fetch the student's competency grading for this lesson
                                                        ...
                                                        @foreach($competencies as $code => $title)
                                                            @php
                                                            $codeLower = strtolower($code); // lowercase for db field

                                                            // Get grade and comment if available
                                                            $selectedCompGrade = $grading?->{$codeLower . '_grade'} ?? null;
                                                            $selectedCompComment = $grading?->{$codeLower . '_comment'} ?? null;
                                                            @endphp

                                                            <div class="custom-box">
                                                                <div class="header" data-bs-toggle="collapse" data-bs-target="#competency-box-{{ $codeLower }}" aria-expanded="false">
                                                                    <span class="rmk">RMK</span>
                                                                    <span class="question-mark">?</span>
                                                                    <span class="title"><span class="highlight">{{ $title }} ({{ strtoupper($codeLower) }})</span></span>
                                                                                        ...
                                                                                        <td>
                                                                                            <label class="radio-label">
                                                                                                <input type="radio" class="scale-radio"
                                                                                                    name="comp_grade[{{ $lesson->id }}][{{ $codeLower }}]"
                                                                                                    value="{{ $i }}"
                                                            ...
                                                            <div class="collapse mt-2" id="competency-box-{{ $codeLower }}">
                                                                <textarea name="comp_comments[{{ $lesson->id }}][{{ $codeLower }}]" rows="3" class="form-control" placeholder="Add remarks or comments on competency">{{ $selectedCompComment }}</textarea>
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
                                                                            ...
                                                                            <tbody>
                                                                                <tr>
                                                                                    @for ($i = 1; $i <= 5; $i++)
                                                                                        ...
                                                                                        $colorClass=$i==1 ? 'incomplete' : ($i==2 ? 'ftr' : 'competent' );
                                                                                        @endphp
                                                                                        <td class="{{ $isStudentSelf ? 'disabled-td' : '' }}">
                                                                                        <label class="radio-label">
                                                                                            <input type="radio" class="scale-radio"
                                                ...
                                                <div id="pilot-{{ $eventLesson->id ?? '' }}" class="accordion-collapse collapse">
                                                    ...
                                                    @if($pilot_cbta)
                                                        <div class="accordion-body">
                                                            ...
                                                            @php
                                                            // Find grading for this competency and lesson
                                                            $savedGrade = collect($pilot_grading)->first(function($g) use ($val, $lesson) {
                                                            return $g['cbta_gradings_id'] == $val['id'] && $g['lesson_id'] == $lesson->id;
                                                            });
                                                            @endphp
                                                                            ...
                                                                            <tbody>
                                                                                <tr>
                                                                                    @for ($i = 1; $i <= 5; $i++)
                                                                                        ...
                                                                                        <label class="radio-label">
                                                                                            <input type="radio" class="scale-radio"
                                                                                                name="pilot_grade[{{ $lesson->id }}][{{ $val['id'] }}]"
                                                                                                value="{{ $i }}"
                                                                                                data-event-id="{{ $trainingEvent->id }}"
                                                                                                data-lesson-id="{{ $lesson->id }}"
                                                                                                data-user-id="{{ $student->id ?? '' }}"
                                    ...
                                    <form action="" method="POST" id="defGradingFrom">
                                        ...
                                        <div class="accordion-item">
                                            ...
                                            <div id="def-lesson-{{ $defLesson?->id }}" class="accordion-collapse collapse show" data-bs-parent="#faq-group-2">
                                                ...
                                                <?php
                                                    $taskIds = $defLesson->task_ids;

                                                    $enable_instructor_cbta = \App\Models\SubLesson::whereIn('id', $taskIds)
                                                        ->whereHas('lesson', function ($query) {
                                                            $query->where('instructor_cbta', 1);
                                                        })
                                                        ->exists();
                                                        
                                                    $enable_examiner_cbta = \App\Models\SubLesson::whereIn('id', $taskIds)
                                                        ->whereHas('lesson', function ($query) {
                                                            $query->Where('examiner_cbta', 1);
                                                        })
                                                        ->exists();

                                                    $enable_pilot_cbta = \App\Models\SubLesson::whereIn('id', $taskIds)
                                                        ->whereHas('lesson', function ($query) {
                                                            $query->Where('pilot_cbta', 1);
                                                        })
                                                        ->exists();

                                                ?>
                                                @if($enable_examiner_cbta)
                                                    <!-- Examiner CBTA -->
                                                    <div class="accordion-card">
                                                        <div class="accordion-item">
                                                            @if($enable_examiner_cbta)
                                                ...
                                                <!-- @if($task->task->courseLesson->course->enable_cbta==1)
                                                    <div class="accordion-card">
                                                        <div class="accordion-item">
                                                            <h2 class="accordion-header">
                                                                <div class="d-flex justify-content-between align-items-center">
                                                                    <button type="button" class="accordion-button collapsed toggleable" data-bs-toggle="collapse" data-bs-target="#comptency-{{ $task->id }}" aria-expanded="false">
                                                                    Overall Competency Grading
                                                                    </button>
                                                                </div>
                                                            </h2>
    ...
    <script>
        $(document).ready(function() {
            ...
            document.getElementById('instructorSelect').addEventListener('change', function () {
                ...
                // If licence already available in option
                if (licence) {
                    ...
                    fetch(`/get-instructor-licence/${instructorId}`)
                        ...
                        .then(data => {
                            let fetchedLicence = data.licence || '';
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '.customTimeSelect', function () {
                ...
                let allotted = parseFloat(selected.data('hours')) || 0;
