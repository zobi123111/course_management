...
<div class="main_cont_outer">
                        ...
                        @if($user->licence_required == 1)
                            <div class="col-md-6 mb-4 license-six-cont">
                                ...
                                <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="non_expiring_licence_uk"  value="1" {{ $document?->licence_non_expiring ? 'checked': '' }}>
                                        <label class="form-check-label" for="non_expiring_licence_uk" data-user-id="{{ $document->user_id }}">
    ...
    <script>
        ...
        $(document).on('change', '#non_expiring_licence_uk', function () { 
                let isChecked = $(this).is(':checked') ? 1 : 0;
                let userId = $(this).next('label').data('user-id');
                $.ajax({
                    url: '/updateNnon_expiringLicence_uk', // your route
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        user_id: userId,
